...
async function createLinkedBookingExcel() {
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Henu Hotel';
  wb.lastModifiedBy = 'Henu Hotel';
  wb.created = new Date();
  wb.modified = new Date();

  // Palette
  const navyHeader = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1E293B' } }; // #1e293b
  const tealHeader = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF0F766E' } }; // #0f766e
  const goldHeader = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFB45309' } }; // #b45309
  ...
  // =========================================================================
  // 1. SHEET 1: حالة الغرف والأسعار
  // =========================================================================
  const ws1 = wb.addWorksheet('حالة الغرف والأسعار', {
  ...
  // 2. الغرف الساكنة (مشغولة حالياً) (C3:D4) - باللون الأزرق
  ...
  // 3. الغرف المحجوزة (E3:F4) - باللون الأحمر/الوردي
  ...
  // 4. الغرف المتاحة (G3:G4) - باللون الأخضر
  ...
  for (let idx = 0; idx < maxRoomsCount; idx++) {
    ...
    // Status: Dropdown with 5 choices including "ساكن"
    const statusCell = row.getCell(5);
    statusCell.value = r ? 'متاحة' : '';
    statusCell.dataValidation = {
      type: 'list',
      allowBlank: true,
      formulae: ['"متاحة,ساكن,محجوزة,تحت التنظيف,صيانة"']
    };
    statusCell.font = { name: 'Segoe UI', size: 10, bold: true };

    // Guest Name: fetched from Bookings sheet
    row.getCell(6).value = {
      formula: `IF(A${rIdx}="","", IFERROR(INDEX('سجل الحجوزات'!$E$4:$E$100, MATCH(A${rIdx}, 'سجل الحجوزات'!$B$4:$B$100, 0)), "-"))`,
      result: r ? '-' : ''
    };

    // Check-in: fetched from Bookings sheet
    row.getCell(7).value = {
      formula: `IF(A${rIdx}="","", IFERROR(INDEX('سجل الحجوزات'!$G$4:$G$100, MATCH(A${rIdx}, 'سجل الحجوزات'!$B$4:$B$100, 0)), "-"))`,
      result: r ? '-' : ''
    };

    // Check-out: fetched from Bookings sheet
    row.getCell(8).value = {
      formula: `IF(A${rIdx}="","", IFERROR(INDEX('سجل الحجوزات'!$H$4:$H$100, MATCH(A${rIdx}, 'سجل الحجوزات'!$B$4:$B$100, 0)), "-"))`,
      result: r ? '-' : ''
    };

    // Paid amount: summed from Bookings sheet
    row.getCell(9).value = {
      formula: `IF(A${rIdx}="","", SUMIF('سجل الحجوزات'!$B$4:$B$100, A${rIdx}, 'سجل الحجوزات'!$L$4:$L$100))`,
      result: r ? 0 : ''
    };
    row.getCell(9).numFmt = '#,##0 "ج.م"';

    // Initial styling: all initial rooms default to soft green
  ...
  ws1.columns = [
    ...
    { width: 22 }, // نوع الغرفة
    { width: 18 }, // سعر الليلة
    { width: 22 }, // حالة الغرفة
    { width: 25 }, // اسم النزيل
    { width: 15 }, // تاريخ الدخول
    { width: 15 }, // تاريخ الخروج
    { width: 20 }, // المبلغ المدفوع
    { width: 25 }  // ملاحظات
  ];

  // =========================================================================
  // Entire Row Conditional Formatting:
  // Formula evaluates =$E8 to change color of the entire row (A8:J60)!
  // =========================================================================
  try {
    ws1.addConditionalFormatting({
      ref: `A8:J${totalRowIdx - 1}`,
      rules: [
        // 1. ساكن (Occupied / In-House) -> Entire Row turns Soft Blue #DBEAFE with Blue text
        {
          type: 'expression',
          formulae: ['=$E8="ساكن"'],
          style: {
        ...
        // 2. محجوزة (Booked / Reserved) -> Entire Row turns Soft Red #FEE2E2 with Dark Red text
        {
          type: 'expression',
          formulae: ['=$E8="محجوزة"'],
          style: {
        ...
        // 3. متاحة (Available) -> Entire Row turns Soft Green #DCFCE7 with Dark Green text
        {
          type: 'expression',
          formulae: ['=$E8="متاحة"'],
          style: {
        ...
        // 4. تحت التنظيف (Cleaning) -> Entire Row turns Soft Yellow/Amber #FEF3C7 with Amber text
        {
          type: 'expression',
        ...
        // 5. صيانة (Maintenance) -> Entire Row turns Soft Gray #F1F5F9 with Slate text
        {
          type: 'expression',
          formulae: ['=$E8="صيانة"'],
          style: {
    ...
    console.log('Row CF note:', e.message);
  }

  // =========================================================================
  // 2. SHEET 2: سجل الحجوزات
  // =========================================================================
  const ws2 = wb.addWorksheet('سجل الحجوزات', {
    views: [{ rightToLeft: true, showGridLines: true }]
  });
  ...
  for (let idx = 1; idx <= totalBookingRows; idx++) {
    ...
    // Room number dropdown list pointing to Sheet 1
    row.getCell(2).value = '';
    row.getCell(2).dataValidation = {
      type: 'list',
      allowBlank: true,
      formulae: ["'حالة الغرف والأسعار'!$A$8:$A$60"]
    };

    // Auto Floor
    row.getCell(3).value = {
      formula: `IF(B${rIdx}="","", IFERROR(VLOOKUP(B${rIdx}, 'حالة الغرف والأسعار'!$A$8:$D$60, 2, FALSE), ""))`
    };

    // Auto Room Type
    row.getCell(4).value = {
      formula: `IF(B${rIdx}="","", IFERROR(VLOOKUP(B${rIdx}, 'حالة الغرف والأسعار'!$A$8:$D$60, 3, FALSE), ""))`
    };
    ...
    // Base Rate
    row.getCell(10).value = {
      formula: `IF(B${rIdx}="","", IFERROR(VLOOKUP(B${rIdx}, 'حالة الغرف والأسعار'!$A$8:$D$60, 4, FALSE), 0))`
    };
    ...
    // Payment method & Booking status dropdown
    row.getCell(14).value = '';
    row.getCell(14).dataValidation = {
      type: 'list',
      allowBlank: true,
      formulae: ['"مؤكد - نقدي,مؤكد - فيزا,مؤكد - فودافون كاش,مؤكد - بوكينج,دفعة مقدمة,معلق,ملغي"']
    };
  ...
  // =========================================================================
  // 3. SHEET 3: دليل أنواع الغرف والأسعار
  // =========================================================================
  const ws3 = wb.addWorksheet('دليل أنواع الغرف والأسعار', {
    views: [{ rightToLeft: true, showGridLines: true }]
  });
  ...
  const destPathPrimary = path.join(__dirname, 'جدول_حجز_الغرف_الذكي_المترابط.xlsx');
  await wb.xlsx.writeFile(destPathPrimary);
  console.log('SUCCESS: File created successfully at ' + destPathPrimary);

  try {
    const destPathSimple = path.join(__dirname, 'جدول_حجز_الغرف_البسيط.xlsx');
    await wb.xlsx.writeFile(destPathSimple);
    console.log('SUCCESS: Also updated ' + destPathSimple);
  } catch (err) {
    console.log('Notice: جدول_حجز_الغرف_البسيط.xlsx is currently open in Excel.');
  }
}

createLinkedBookingExcel().catch(err => {
