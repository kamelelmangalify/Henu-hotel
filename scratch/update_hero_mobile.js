...
const perfectedHomeHtml = `<!-- wp:html -->
...
.henu-hero-desktop-wrap {
  background-image: url('${uploadsBase}/hero-pyramids-sunset.jpg');
...
.henu-hero-title span { color: var(--clr-gold-bright); }
...
/* ========================================================
   MOBILE RESPONSIVE (SMART UNCROPPED HERO & CRISP CARDS)
   ======================================================== */
...
@media (max-width: 768px) {
  ...
  /* MOBILE HERO: CRISP UNCROPPED PANORAMIC PHOTO */
  .henu-hero-desktop-wrap {
  ...
  .henu-hero-card {
    background: rgba(11, 27, 43, 0.95) !important;
    padding: 16px !important;
    border-radius: 14px !important;
    border: 1px solid rgba(201, 135, 58, 0.4) !important;
...
<!-- HERO SECTION -->
<div class="henu-hero-desktop-wrap">
  <div class="henu-hero-card">
    
    <!-- Mobile Panoramic Image (Shows full uncropped Pyramids view on phone) -->
    <div class="henu-hero-mobile-img-box">
      <img src="${uploadsBase}/hero-pyramids-sunset.jpg" alt="HENU Pyramids Sunset View" class="henu-hero-mobile-img">
    </div>

    <span class="henu-badge">★ Pyramids View Boutique Hotel · Giza</span>
    <h1 class="henu-hero-title">Where Ancient Wonders Meet <span>Modern Comfort</span></h1>
    <p class="henu-hero-desc">
    ...
    <div style="display: flex; gap: 14px; justify-content: center; flex-wrap: wrap;">
      <a href="/contact#booking" class="btn-henu-gold">📅 Instant WhatsApp Booking</a>
      <a href="/accommodation" class="btn-henu-outline">🛏️ View Rooms from $12</a>
    </div>
  </div>
</div>
...
const localPath = 'd:/Henu/scratch/pages/home_hero_fixed.html';
fs.writeFileSync(localPath, perfectedHomeHtml, 'utf8');

const conn = new Client();
conn.on('ready', () => {
  console.log('SSH Connected. Uploading fixed mobile hero home page...');
  conn.sftp((err, sftp) => {
    if (err) throw err;

    sftp.fastPut(localPath, 'domains/henuhotel.com/public_html/p_home_hero_fixed.html', (err) => {
      if (err) throw err;

      const cmd = `
        cd ~/domains/henuhotel.com/public_html
        HOME_ID=$(wp post list --post_type=page --name=home --field=ID)
        wp post update $HOME_ID p_home_hero_fixed.html
        rm -f p_home_hero_fixed.html

        # Clear LiteSpeed Cache
        wp litespeed-purge all 2>/dev/null || true
        echo "=== MOBILE HERO FIXED AND VERIFIED ==="
      `;
