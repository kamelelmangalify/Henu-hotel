...
const multilanguageNavbarPhp = `<?php
/**
...
fs.writeFileSync(localPhp, multilanguageNavbarPhp, 'utf8');

const conn = new Client();
conn.on('ready', () => {
  console.log('SSH Connected. Uploading Multilingual Navbar & Footer...');
  conn.sftp((err, sftp) => {
    if (err) throw err;
    ...
    sftp.fastPut(localPhp, 'domains/henuhotel.com/public_html/wp-content/mu-plugins/henu-navbar.php', (err) => {
      ...
      const cmd = `
        ...
        echo "=== MULTILINGUAL SWITCHER INSTALLED AND VERIFIED ==="
      `;
